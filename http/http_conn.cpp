...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_url++ = '\0';
    ...
    // strspn返回字符串中第一个不在指定字符串中出现的字符的位置
    m_url += strspn(m_url, " \t");
    // strpbrk返回字符串中第一个在指定字符串中出现的字符的位置
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    {
        LOG_INFO("Oops!, unknow header: %s", text);
    }
    return NO_REQUEST;
}
...
// do_request函数被process_read调用，处理HTTP请求
...
bool http_conn::write()
{
    ...
    if(bytes_to_send == 0)
    {
        // 如果没有数据需要发送，则需要修改EPOLLONESHOT事件，以确保下一次有数据发送时，EPOLLIN事件能够触发
    ...
    while (1)
    {
        ...
        if (bytes_have_send >= m_iv[0].iov_len)
        {
            // 第一个iovec头部信息发送完
            m_iv[0].iov_len = 0;
            // 第二个iovec发送文件 为文件地址加上已发送字节数减去写索引
